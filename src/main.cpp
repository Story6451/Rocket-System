...
void RetrieveGyroData()
{
  ...
  Gyroscrope.update();
  AngleRoll = Gyroscrope.getAngleX();
  AnglePitch = Gyroscrope.getAngleY();
  AngleYaw = Gyroscrope.getAngleZ();
}
...
void loop() 
{
  ...
  if (Launched == true)
  {
    ...
    if ((DeltaAltitude < (DeltaMaxAltitude - AltitudeUncertainty)) || ((millis() - MainStartTime) > 5000))
    {
